...
import dev.rajaopak.universenick.api.NickResetEvent;
import dev.rajaopak.universenick.api.NickResetOtherEvent;
...
/**
 * Handles <code>/nickreset</code> command execution and tab completion.
 */
public class CommandNickReset implements TabExecutor {
  ...
  public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
    ...
    if (args.length == 0) {
      ...
      // Call event
      NickResetEvent nickResetEvent = new NickResetEvent(player,
          Nicks.core().getDisplayName(player));
      Nicks.api().callEvent(nickResetEvent);
      if (nickResetEvent.isCancelled()) {
        return true;
      }
      ...
      // Call event
      NickResetOtherEvent event = new NickResetOtherEvent(sender, target,
